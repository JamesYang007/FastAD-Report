...
Our benchmark drivers are very similar to the one used by Stan~\cite{carpenter:2015}.
...
\end{lstlisting}
\fi
All functors overload \code{operator()} to take in a library-specific vector of AD variables.
...
which will always try to use \code{Eigen} API.
If a function cannot be easily written with \code{Eigen} API, then a manually written code is used.
...
% Original: have code for how we benchmarked
\if 0
...
% Original: only makes sense if code is shown above
\if 0
...
% Simplified: broad description of only what is necessary
We measure the average time over $T$ iterations
...
% Original: only makes sense if adept code is shown
\if 0
