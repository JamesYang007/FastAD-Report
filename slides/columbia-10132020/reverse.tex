...
\begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-0.5,xscale=0.5]
    ...
    \pause%
    \draw [red] (sub) -- (log) node[midway, label={[red]right:$-a_{10}$}];

    \pause%
    \draw [red] (log) -- (w4) node[midway, label={[red]right:$\frac{a_9}{w_4}$}];

    \pause%
    \draw [red, dashed] (w4) -- (x3) node[midway, label={[red]280:$\frac{a_9}{w_4}$}];

    \pause%
    \draw [red] (sub) -- (add) node[midway, label={[red]100:$a_{10}$}];

    \pause%
    \draw [red] (add) -- (mul) node[midway, label={[red]90:$a_{8}$}];

    \pause%
    \draw [red] (mul) -- (w3) node[midway, label={[red]-20:$a_{7} \cdot w_6$}];

    \pause%
    \draw [red, dashed] (w3) -- (x3) node[midway, label={[red]left:$a_{7} \cdot w_6$}];

    \pause%
    \draw [red] (mul) -- (cos) node[midway, label={[red]100:$a_{7} \cdot w_3$}];

    \pause%
    \draw [red] (cos) -- (w2) node[midway, label={[red]left:\scriptsize $-a_{6}\sin(w_2)$}];

    \pause%
    \draw [red, dashed] (w2) -- (x2) node[midway, label={[red]left:\scriptsize $-a_{6}\sin(w_2)$}];

    \pause%
    \draw [red] (add) -- (sin) node[midway, label={[red]100:$a_8$}];

    \pause%
    \draw [red] (sin) -- (w1) node[midway, label={[red]left:\scriptsize $a_5\cos(w_1)$}];

    \pause%
    \draw [red, dashed] (w1) -- (x1) node[midway, label={[red]left:\scriptsize $a_5\cos(w_1)$}];
