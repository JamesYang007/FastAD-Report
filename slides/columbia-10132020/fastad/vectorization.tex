...
\item Why not integrate a good matrix library? 
    \begin{itemize}
        \item \code{Eigen} (industry standard)
    \end{itemize}
...
\item Stan integrates \code{Eigen}, but there are examples where it does not vectorize well.
...
\item Stan has ``Plug-ins'' that extend \code{Eigen} to work with Stan AD variables.
    \begin{itemize}
